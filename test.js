...
test('Slugify remove whitespaces', t => {
  // Remove Symbols
  t.is(slugify(' no extra spaces  '), 'no-extra-spaces');
});
...
test('Slugify all lowercase', t => {
  // Remove Symbols
  t.is(slugify('ALL LOWERCASE'), 'all-lowercase');
});

test('Slugify remove symbols (except dash)', t => {
  // Remove Symbols
  t.is(slugify('no $ymbols allowed!@#$%^&*()=-'), 'no-ymbols-allowed-');
});
...
test('Slugify alphabet allowed', t => {
  // Remove Numbers
  t.is(slugify('abcdefghijklmnopqrstuvwxyz'), 'abcdefghijklmnopqrstuvwxyz');
});

test('Slugify numbers allowed', t => {
  // Remove Numbers
  t.is(slugify('0123456789'), '0123456789');
});
